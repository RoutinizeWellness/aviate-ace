...
b737Categories.forEach(category => {
  // Generate 500 questions for each category
  for (let i = 1; i <= 500; i++) {
    allQuestions.push(generateQuestion(category, i, 'BOEING_737'));
  }
});
...
// Write to file
const fileName = 'generated-b737-questions.cjs';
fs.writeFileSync(fileName, `// Generated BOEING_737 questions - 500 per category
const generatedB737Questions = [
${output}
];

module.exports = { generatedB737Questions };
`);

console.log(`Generated ${allQuestions.length} questions for BOEING_737 across ${b737Categories.length} categories`);
